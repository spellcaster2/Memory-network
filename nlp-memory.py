...
def get_data(challenge_type):
  ...
  word2idx = {c:i for i, c in enumerate(vocab)}  inputs_train, queries_train, answers_train = vectorize_stories(
    train_stories,
    word2idx,
    story_maxlen,
    query_maxlen
...
embedded_question = Lambda(lambda x: K.sum(x, axis=1))(embedded_ques
embedded_question = Reshape((1, embedding_dim))(embedded_question)
print("inp_q.shape, emb_q.shape:", input_question_.shape, embedded_question.shax = dot([embedded_story, embedded_question], 2)
...
ans = Dense(vocab_size, activation='softmax')model = Model([input_story_, input_question_], model.compile(
  optimizer=RMSprop(lr=1e-2),
  loss='sparse_categorical_crossentropy',
  metrics=['accuracy']r = model.fit(
  [inputs_train, queries_train],
  answers_train,
  epochs=4,
  batch_size=32,
  validation_data=([inputs_test, queries_test], answers_test)
)debug_model = Model([input_story_, input_question_], story_weights)story_idx = np.random.choice(len(train_stories))
...
def hop(query, story):  x = Reshape((1, embedding_dim))(query)
  x = Reshape((story_maxsents,))(x)
  x = Activation('softmax')(x)
  story_weights = Reshape((story_maxsents, 1))(x)
